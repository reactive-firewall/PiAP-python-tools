...
INTERFACE_CHOICES=[str('{}{}').format(x, str(y)) for x in ['wlan','eth', 'mon'] for y in range(5)]
...
def error_handling(func):
	...
	def helper(**kwargs):
		...
		try:
			...
			print(str(err))
			print(str((err.args)))
			print(str("{}: REALLY BAD ERROR: ACTION will not be compleated! ABORT!").format(__prog__))
			theOutput = None
		return theOutput
	return helper
...
def parseargs():
	...
	try:
		...
		parser.add_argument('-i', '--interface', default=INTERFACE_CHOICES[1], choices=INTERFACE_CHOICES, help='The interface to show.')
		parser.add_argument('-l', '--list', default=False, action='store_true', help='List current interfaces.')
		parser.add_argument('--html', dest='output_html', default=False, action='store_true', help='output html.')
		parser.add_argument('-a', '--all', dest='show_all', default=False, action='store_true', help='show all interfaces.')
	...
	if is_verbose is True:
		...
		try:
			if use_html:
				...
				format_pattern = u'{} {} {} {}'
			theResult = gen_html_tr(format_pattern.format(
...
def extractIfaceNames(theInputStr):
	...
	return extractRegexPattern(theInputStr, "(?:(?:[[:print:]]*){0,1}(?P<iface_name>[aehltw]{3}[n]?[0-9]+){1}(?:[[:print:]]*){0,1})+")
...
def extractFieldsAddr(theInputStr):
	"""Extract the fields from a raw status string."""
	return None
	#return extractRegexPattern(theInputStr, "(?:(?:(?P<interface_index>[0-9]+){1}(?:[\:]{1}\s+){1}(?P<interface_name>[a-z0-9.]+){1}(?:[\:]{1}\s+[<]{1}){1}(?P<interface_flags>(?:(?P<interface_flag>LOOPBACK|UP|LOWER_UP|DOWN|BROADCAST|MULTICAST|PROMISC|LOWER_DOWN|NO-CARRIER){1}(?:[,]{1})?)+){1}(?:[\>]{1}\s+){1}(?:[mtu]{3}\s+){1}(?P<interface_mtu>[0-9]+){1}(?:\s+){1}(?:(?:\s*){1}(?P<interface_attr>[^ ]+){1})*(?:\s*){1}(?:[s]{1}[t]{1}[a]{1}[t]{1}[e]{1}\s+){1}(?P<interface_state>UP|DOWN|UNKNOWN){1}(?:\s+)?(?:(?:[group]{5}\s+){1}(?P<interface_group>[^ ]+){1})*(?:(?:[^ ]+\s+){1}(?P<interface_queue_flags>[^ ]+){1})*(?:\s+)+(?:[link]{4}[/]{1}){1}(?P<interface_link_type>loopback|ether)(?:\s+){1}(?P<interface_mac>(?:(?:[0-9a-fA-F]{1,2}[\:]{1}){5}(?:[0-9a-fA-F]{1,2}){1}){1}){1}(?:\s+[brd]{3}\s+){1}(?P<brd_mac>(?:(?:[0-9a-fA-F]{1,2}[\:]{1}){5}(?:[0-9a-fA-F]{1,2}){1}){1}){1}(?:\s+)?){1}(?P<interface_addr>(?:(?:\s+)*(?P<interface_addr_type>[i]{1}[n]{1}[e]{1}[t]{1}[6]?)(?:\s+){1}(?P<interface_addr_ip>(?:(?:[0-9]{1,3}[\.]{1}){3}(?:[0-9]{1,3}){1}|(?:[:]{1,2}[0-9a-fA-F]{1,4})+){1}(?:[/]?[0-9]{1,3})?){1}(?:(?:\s+[brd]{3}\s+){1}(?P<interface_addr_brd>(?:(?:[0-9]{1,3}[\.]{1}){3}(?:[0-9]{1,3}){1}|(?:[:]{1,2}[0-9a-fA-F]{1,4})+){1}){1})?(?:(?:\s+[s]{1}[c]{1}[o]{1}[p]{1}[e]{1}\s+){1}(?P<interface_addr_scope>host|link|site|global|local){1}(?:\s+){1}(?:(?:\s*){1}(?P<interface_addr_attr>[^ ]+){1})*)?(?:(?:\s+){1}(?:[valid]{5}[_lft]{4}\s+){1}(?P<interface_addr_valid_lft>[^ ]+){1}(?:\s*){1}(?:[prefd]{9}[_lft]{4}\s+){1}(?P<interface_addr_pref_lft>[^ ]+){1}(?:\s+)*)?){1}(?:\s+)*)+)+")
...
def get_iface_list():
	...
	try:
		...
		theResult = compactList([x for x in extractIfaceNames(theRawIfaceState) if x in INTERFACE_CHOICES])
		...
		if use_html is False:
			...
			if status_txt is not None:
				if (u' DOWN' in status_txt):
					theResult = gen_html_td(gen_html_label(u'DOWN', u'danger'), str(u'iface_status_value_{}').format(iface))
				elif (u' UP' in status_txt):
					theResult = gen_html_td(gen_html_label(u'UP', u'success'), str(u'iface_status_value_{}').format(iface))
				else:
					theResult = gen_html_td(gen_html_label(u'UNKNOWN', u'default'), str(u'iface_status_value_{}').format(iface))
...
def get_iface_mac(iface=u'lo', use_html=False):
	...
	try:
		...
		if use_html is False:
			...
			theResult = gen_html_td(get_iface_mac(iface, False), str(u'iface_status_mac_{}').format(iface))
	except Exception as errcrit:
		print(str(errcrit))
		print(str(errcrit.args))
		print(str(extractIfaceNames(theRawIfaceState)))
...
def get_iface_ip_list(iface=u'lo', use_html=False):
	...
	try:
		...
		if use_html is False:
			...
			if ip_list_txt is not None and len(ip_list_txt) > 0:
				theResult = gen_html_td(gen_html_ul(ip_list_txt), str(u'iface_status_ips_{}').format(iface))
			else:
				theResult = gen_html_td(gen_html_label(u'No IP', HTML_LABEL_ROLES[3]), str(u'iface_status_ips_{}').format(iface))
	except Exception as errcrit:
		print(str(errcrit))
		print(str(errcrit.args))
		print(str(extractIfaceNames(theRawIfaceState)))
...
def main():
	...
	try:
		...
		if args.show_all is True:
			if output_html:
				print("<table class=\"table table-striped\"><thead><th>Interface</th><th>MAC</th><th>IP</th><th>Status</th></thead><tbody>")
