...
__prog__ = u'clients_check_status'
"""The Program's name"""
...
def parseargs():
	...
	try:
		...
		parser.add_argument('-l', '--list', default=False, action='store_true', help='List current clients.')
		parser.add_argument('--html', dest='output_html', default=False, action='store_true', help='output html.')
		parser.add_argument('-a', '--all', dest='show_all', default=False, action='store_true', help='show all clients.')
...
def get_client_arp_status_raw(client_ip=None):
	...
	try:
		...
		try:
			theRawClientState = subprocess.check_output(arguments, stderr=subprocess.STDOUT)
			if client_ip is not None and theRawClientState is not None and len(theRawClientState) > 0:
				lines = [str(u'{}\n').format(x) for x in theRawClientState.split(u'\n') if client_ip in x]
				theRawClientState = u''
				for line in lines:
					theRawClientState = str(u'{}{}').format(theRawClientState, line)
				del lines
			else:
		...
		if use_html is not True:
			...
			if status_txt is not None:
				if (u' DOWN' in status_txt):
					theResult = gen_html_td(gen_html_label(u'disassociated', u'danger'), str(u'client_status_value_{}').format(client))
				elif (u' UP' in status_txt):
					theResult = gen_html_td(gen_html_label(u'associated', u'success'), str(u'client_status_value_{}').format(client))
				else:
					theResult = gen_html_td(gen_html_label(u'UNKNOWN', u'default'), str(u'client_status_value_{}').format(client))
...
def get_client_mac(client=None, use_html=False):
	...
	try:
		...
		if use_html is not True:
			...
			theResult = gen_html_td(get_client_mac(client, False), str(u'client_status_mac_{}').format(client))
	except Exception as errcrit:
		print(str(errcrit))
		print(str(errcrit.args))
		theResult = None
...
def get_client_ip(client=None, use_html=False):
	...
	try:
		...
		if use_html is not True:
			...
			if ip_list_txt is not None and len(ip_list_txt) > 0:
				theResult = gen_html_td(gen_html_ul(ip_list_txt), str(u'client_status_ips_{}').format(client))
			else:
				theResult = gen_html_td(gen_html_label(u'No IP', HTML_LABEL_ROLES[3]), str(u'client_status_ips_{}').format(client))
	except Exception as errcrit:
		print(str(errcrit))
		print(str(errcrit.args))
		theResult = None
...
def main():
	...
	try:
		...
		if args.show_all is True:
			if output_html:
				print("<table class=\"table table-striped\"><thead><th>Client</th><th>MAC</th><th>IP</th><th>Status</th></thead><tbody>")
